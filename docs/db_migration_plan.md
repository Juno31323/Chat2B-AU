...
- H2/local fallback: `document_chunks.embedding` text 컬럼에 vector literal 문자열을 저장한다.
- PostgreSQL + pgvector 가능 시: `document_chunks.embedding_vector vector(n)` 컬럼을 추가하고 cosine distance top-k 검색에 사용한다.
- 현재 호환 레이어에서는 `documents`, `document_chunks`, `chat_logs`, `index_metadata` 테이블을 사용한다.
...
초안 SQL은 `src/main/resources/db/migration/postgres/V1__create_rag_experiment_schema.sql`에 둔다.
...
- `embedding_vector vector(256)` 생성 여부
- HNSW index 생성 여부
